...
import ar.edu.um.ingenieria_aplicada.microblogging.repository.UsuarioRepository;
...
public class UsuarioResource {
    ...
    @GetMapping("/usuarios")
    @Timed
    public ResponseEntity<List<Usuario>> getAllUsuarios(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Usuarios");
        Page<Usuario> page;
        if (eagerload) {
            page = usuarioRepository.findAllWithEagerRelationships(pageable);
        } else {
            page = usuarioRepository.findAll(pageable);
        }
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, String.format("/api/usuarios?eagerload=%b", eagerload));
